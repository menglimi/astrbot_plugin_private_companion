...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.6.10 汇总私聊图片检测、指令注册诊断、主动发送兜底和 QQ 空间配图/评论回复排障优化。
...
- 凭据来源优先级：先使用手动 `QZONE_COOKIE`，留空时再尝试从当前 OneBot 连接自动获取 Cookie。自动模式要求 QQ 平台连接已经在线，且 OneBot 实现支持 `get_cookies`、`get_credentials` 或 `call_action` 调用；插件会从最近消息事件、`aiocqhttp` 平台实例和平台管理器里寻找可用连接。
...
如果希望临时提高主动频率，可以使用“主动强度预设”。它默认关闭，关闭时完全沿用手动配置；开启后只作为运行态覆盖层影响私聊主动、群聊唤醒和群聊插话的有效频率，不会改写下方手动参数，也不会绕过免打扰、休息静默、用户拒绝、隐私边界和成本闸门。预设启用时会在排障页显示当前档位和关键有效值，方便确认为什么最近主动变多或变少。
